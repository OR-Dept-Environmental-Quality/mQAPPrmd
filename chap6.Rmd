...
if(!model.extent == TRUE){
  ...
  t6x5 <- section.model.input %>% 
    ...
    dplyr::mutate(Parameter = ifelse(`Station ID` == "TIR", paste0(Parameter," (TIR)"), Parameter),
                  `Station ID` = ifelse(`Station ID` == "TIR","",`Station ID`),
    ...
    dplyr::select(`Model Location Name (Station ID)`, `Model_Location`,  `Parameter`, `Data Source`) %>% 
...
* The data source and quality for the following boundary condition inputs: `r if(NROW(t6x6_1)>0){paste(unique(sort(t6x6_1$"Model Location Name (Station ID)"), collapse = ", "))}else{cat("No data")}`. **[UPDATE FOR EACH STREAM]**
