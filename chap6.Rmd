...
npdes.ind.waterbody <- npdes.ind.pro.area %>%
  ...
  dplyr::mutate(`Stream/River Mile` = ifelse(`Facility Name (Facility Number)` == "Ashland STP (3780)", "Ashland Creek RM 0.2 (current); Bear Creek RM 22.8 (future)",`Stream/River Mile`)) # Rogue
...
if(npdes.ind.n > 1) {
  npdes.txt <- ifelse(!waterbody_name == "Bear Creek", paste0("There are ", numbers.to.words(npdes.ind.n)," permitted individual NPDES point sources along the model extent. Detail about each point source is summarized in ",tbls(name = paste0("npdes.ind.waterbody_",waterbody_name),display="cite"),"."), paste0("There is one permitted individual NPDES point source along the model extent. Detail about the point source is summarized in ",tbls(name = paste0("npdes.ind.waterbody_",waterbody_name),display="cite"), ". The City of Ashland (#3780) owns and operates a wastewater treatment facility that discharges treated effluent into Ashland Creek upstream of its confluence with Bear Creek at river mile 0.2. Currently there are plans to move the outfall to Bear Creek approximately 100 feet downstream from the confluence of Ashland Creek at about river mile 22.8. DEQ may consider this new outfall in the Bear Creek model scenarios."))
} 
...
if(waterbody_name %in% c("Antelope Creek","Bear Creek","Little Applegate River",
  ...
  sc_txt <- paste0("reductions to stream flow rate or volume,",ifelse(waterbody_name == "Bear Creek"," water impoundments behind irrigation structures, "," "),"and background sources (",section.model.info$Cite_inText,"). Other potential sources include channel modification and widening and warming caused by climate change. ")
...
`r if(waterbody_name %in% c("Applegate River")){paste0("There is one permitted individual NPDES point source along the model extent (Hidden Valley High School STP). Detail about this permittee is summarized in Table 18. The current NPDES permit for Hidden Valley High School STP does not authorize discharge May 1st – October 31st so they are not a source of warming during the summer period.")}`

`r if(!waterbody_name %in% c("Applegate River")){npdes.txt}`
