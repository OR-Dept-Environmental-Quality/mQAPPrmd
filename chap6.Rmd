...
The primary sources of thermal loading contributing to temperatures exceedances in `r paste0(the,waterbody_name)` include increases in solar radiation loading from the disturbance or removal of near-stream vegetation, `r if(npdes.ind.n>0){paste0("point source discharges,")}` `r if(!is.na(source.char.1)){source.char.1}` and background sources (`r strip_alpha(section.model.info$"Abbreviated Reference")`). `r source.char.2` The contribution of these latter potential sources may be investigated as part of the model scenarios.
