...
`r if(waterbody_name == "Columbia Slough"){paste0("This model was calibrated using data from 1992. Based on conversations with the Multnomah County Drainage District, conditions affecting water temperature in the Columbia Slough have changed significantly since then. The Middle and Upper Slough were dredged to make the channel deeper and sediment deposition is expected to have changed channel depth. In addition, pumps controlling stream flow were replaced and the pumping capacity was increased. Due to the extensive morphology and flow changes in the system, the existing model must be updated and recalibrated before it is used simulate current water temperature conditions. The recommended model updates are summarized in Appendix C. Due to resource constraints and the rigorous project schedule, the Columbia Slough CE-QUAL-W2 model will not be updated and used for TMDL development at this time. The existing model is summarized in this section to document its existence and that it was considered for TMDL development.")}`
...
`r if(waterbody_name == "Columbia Slough"){paste0("The primary sources of thermal loading contributing to temperatures exceedances along the Columbia Slough include increases in solar radiation loading from the disturbance or removal of near-stream vegetation, point source discharges, channel modification and widening, and background sources (Berger, 2003). Other potential sources include reductions to stream flow rate and warming caused by climate change.")}`

`r if(waterbody_name == "Johnson Creek"){paste0("The primary sources of thermal loading contributing to temperatures exceedances along the Johnson Creek include increases in solar radiation loading from the disturbance or removal of near-stream vegetation, reductions to the stream flow rate or volume, channel modification, reductions to stream flow, and background sources (DEQ, 2006). Other potential sources include warming caused by climate change. The contribution of these later sources will be investigated as part of the model scenarios.")}`

`r if(waterbody_name == "Columbia Slough"){paste0("Over the years the Slough system has been extensively dredged, diked, filled and channelized, principally by the U.S. Army Corps of Engineers, the City of Portland and the Port of Portland. Originally a series of wetlands and marshes created by annual flooding of the Columbia and Willamette Rivers, the Slough is now a highly managed water system with dikes and pumps to provide watershed drainage and flood control for the lowlands surrounding it.")}`
...
`r if(waterbody_name == "Columbia Slough"){paste0("The Multnomah County Drainage District (MCDD) is a special purpose district whose primary responsibility is to provide flood control for most of the Slough watershed. Due to the extensive modifications to the Columbia Slough, the area within MCDD’s boundaries no longer drains naturally, but relies on two primary pump stations that lift water over the levee, and into the Columbia River and/or lower Columbia Slough which drains to the Willamette River. The hydraulic management of the Slough can have a significant impact on the water quality and uses supported by the Slough (DEQ 2006).  For this reason, the DEQ expects to name MCDD a responsible person for temperature TMDL implementation.")}`

`r tbls(name = paste0("dma.waterbody",waterbody_name),display="cite")` summarizes the potential designated management agencies and responsible persons along the `r waterbody_name` model extent. `r if(waterbody_name == "Columbia Slough"){paste0("This table is based on land ownership, which does not indicate special district status. The MCDD area is expected to overlap with much of the City of Portland, Port of Portland, City of Gresham and City of Fairview land area.")}`
...
if(!model.extent == TRUE){
  ...
  t6x7 <- section.model.input %>% 
    ...
    dplyr::mutate(`Data Source` = ifelse(`Station ID` %in% "TIR", strip_alpha(`Data Source`), `Data Source`),
                  ...
                  `Model Location Name` = ifelse(`Model Location Name` %in% c("Mouth to Headwaters","Headwaters to mouth", "Johnson Creek mouth to headwaters"),"Model extent",`Model Location Name`),
