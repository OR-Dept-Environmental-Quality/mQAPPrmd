...
# for test: waterbody_name <- "John Day River"
...
if(!waterbody_name %in% c("John Day River")){
...
The primary sources of thermal loading contributing to temperatures exceedances in `r paste0(the,waterbody_name)` include increases in solar radiation loading from the disturbance or removal of near-stream vegetation, `r if(npdes.ind.n>0){paste0("point source discharges, ")}` `r sc_txt` The contribution of these latter potential sources may be investigated as part of the model scenarios. 
...
if(!model.extent == TRUE){
  ...
  if(!qapp_project_area == "Willamette River Mainstem and Major Tributaries"){
    ...
    t6x6_met <- section.model.input %>% 
      ...
      dplyr::mutate(Note = "",
                    `Data Source` = str_replace(`Data Source`, "Derived data. ", "")) %>% 
      dplyr::group_by(`Station ID`,`Model Location Type`, `Location Units`, `Data Source`,Note) %>% 
      dplyr::summarize(`Model Location` = toString(unique(sort(round(as.numeric(`Model Location`),1)))),
    ...
    t6x6_1 <- section.model.input %>% 
      ...
      dplyr::filter(!`Model Location Name` == "Accretion") %>% # specific to Middle Fork John Day River
      dplyr::mutate(`Model Location Name (Station ID)` = ifelse(`Station ID` %in% "No Station ID" | is.na(`Station ID`),
                    ...
                    `Model Location` = round(as.numeric(`Model Location`),1),
                    `Data Source` = ifelse(substr(`Data Source`,1,18) == "Watershed Sciences", 
                                           strip_alpha(`Data Source`), `Data Source`),
                    `Data Source` = ifelse(substr(`Data Source`,1,18) == "Watershed Sciences", 
                                           paste0(gsub(",.*$", "", `Data Source`)," (",stringi::stri_sub(`Data Source`,-4),")"), `Data Source`),
