...
`r if(!is.na(unique(section.model.input$"Veg Notes"))){paste0("Near stream vegetation inputs to the model include vegetation height and canopy cover. ", unique(section.model.input$"Veg Notes"))}`
...
if(!model.extent == TRUE){
  ...
  t6x7 <- section.model.input %>% 
    ...
    dplyr::mutate(`Data Source` = ifelse(`Station ID` %in% "TIR", strip_alpha(`Data Source`), `Data Source`),
                  ...
                  `Model Location Name (Station ID)` = ifelse(`Model Location Name (Station ID)` == c("Mouth to Headwaters","Headwaters to mouth"),"Model extent",`Model Location Name (Station ID)`) %>% 
