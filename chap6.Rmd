...
section.tmdl <- section.model.info %>% 
  dplyr::filter(`Primary Model Parameter` == "Temperature") %>% 
...
```{r, label=`hs_v`, echo=FALSE}

if(NROW(section.tmdl)>0) {
  
  child=paste0("hs",hsv.num,".Rmd")
  hs.v <- knitr::knit_child(child, quiet = TRUE)
  cat(hs.v, sep = "\n")
...
`r if(NROW(section.tmdl)>0) {hsv_tbl}`
