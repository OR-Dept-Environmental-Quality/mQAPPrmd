...
section.model.input <- model.input %>% 
  dplyr::filter(`Model Waterbody` %in%  waterbody_name) %>% 
  dplyr::filter(!`Model Location Name` == "Node 1") # branch edit
...
`r if(waterbody_name %in% c("Ramsey Creek","Eightmile Creek")){paste0("The primary purpose of the ", waterbody_name, " solar model was to characterize the status of effective shade. Effective shade is a surrogate for solar radiation loading caused by the disturbance or removal of near-stream vegetation. Other potential sources of thermal loading were not evaluated by this model.")}`

`r if(waterbody_name %in% c("Fifteenmile Creek")){paste0("The primary sources of thermal loading contributing to temperatures exceedances along the Fifteenmile Creek include increases in solar radiation loading from the disturbance or removal of near-stream vegetation, point source discharges, reductions to the stream flow rate or volume, and background sources (DEQ, 2008). Other potential sources include channel modification and widening, and warming caused by climate change. The contribution of these latter potential sources may be investigated as part of the model scenarios.")}`

`r if(waterbody_name %in% c("Fifteenmile Creek")){npdes.txt}`

`r if(waterbody_name %in% c("Fifteenmile Creek")){if(npdes.ind.n > 0){npdes.ind.tbl}}`
...
if(!model.extent == TRUE){
  ...
  if(waterbody_name %in% c("Fifteenmile Creek")){
...
`r if(!model.extent == TRUE){} else {if(waterbody_name %in% c("Fifteenmile Creek")){if(NROW(t6x6_1)>0){paste0(tbls(name = paste0("t6x6_1", waterbody_name),display="cite")," summarizes the current configuration of the model input parameters and the source of these data. Temperature, flow, and meteorological input parameters are summarized to improve documentation of the TMDL approach.")}}}`

`r if(waterbody_name %in% c("Ramsey Creek","Eightmile Creek")){paste0("The inputs to the model include aerial imagery derived vegetation heights and stream position.")}`

`r if(!model.extent == TRUE){paste(chap6x6, collapse = "\n")}else{if(waterbody_name %in% c("Fifteenmile Creek")){if(NROW(t6x6_1)>0){t6x6_1_tbl}}}`
...
`r if(!model.extent == TRUE){} else {if(waterbody_name %in% c("Fifteenmile Creek")){if(NROW(t6x7)>0){paste0("The expected model calibration sites and data sources are summarized in ",tbls(name = paste0("t6x7", waterbody_name),display="cite"),". ", mod.loc[1], if(NROW(t57)>0){paste0(" Effective shade model calibrations sites are summarized in ", tbls(name = "t57", display="cite"), ".")}," The model inputs and parameters that are expected to be modified to improve model fit are described in Section 6.1.")}}}`

`r if(waterbody_name %in% c("Ramsey Creek","Eightmile Creek")){paste0("The model was calibrated by comparing the model effective shade predictions to the field measured effective shade values summarized in ", tbls(name = paste0("t6x7", waterbody_name),display="cite"), ". ", mod.loc[1], " Two or three measurements were taken at each location and averaged for a reach measurement. The model inputs and parameters that are expected to be modified to improve model fit are described in Section 6.1.")}`
