...
# For testing: waterbody_name <- "Cavitt Creek"
# For testing: waterbody_name <- "Antelope Creek"
  ...
  t6x5_1 <- section.model.input %>% 
    ...
    dplyr::mutate(DataSource = ifelse(`Interpolated Data` == "T",
                                      ...
                                      ifelse(!is.na(`Data Source`), 
                                             ...
                                             NA))) %>% 
    dplyr::arrange(Parameter,desc(`Model Location`)) %>% 
    dplyr::select(Location, `Model_Location`, `Site Type`, `Parameter`, DataSource) %>%
    dplyr::rename(`Model Location` = `Model_Location`,
                  `Input Type` = `Site Type`,
...
t6x5_2 <- section.model.input %>%
  dplyr::filter(`Site Type` == "Meteorological")
...
Hourly meteorology inputs into the model include `r tolower(knitr::combine_words(unique(sort(t6x5_2$Parameter))))`. The data source for these inputs is the `r met.org` weather station at the `r met.site`. Air temperature data were modified using the dry adiabatic lapse rate to adjust for differences in elevation between the measurement location and the model input location. Wind speeds were adjusted using a wind-sheltering coefficient to represent difference in wind speed between the measurement location and above the stream within the riparian area. `r unique(sort(t6x5_2$Note))`
...
* The data source and quality for the following boundary condition inputs: `r paste(unique(sort(t6x5_1$"Location"), collapse = ", "))`. **[UPDATE FOR EACH STREAM]**
