...
# for test: waterbody_name <- "Pudding River"
...
## 1st paragraph
source.char.1 <- readxl::read_xlsx("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/tables_MidWillamette.xlsx",
                                 sheet = "midW_sourceChar") %>%
  dplyr::filter(`Model Waterbody` == waterbody_name) %>% 
  dplyr::pull(sourceChar1)

source.char.2 <- readxl::read_xlsx("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/tables_MidWillamette.xlsx",
                                 sheet = "midW_sourceChar") %>%
  dplyr::filter(`Model Waterbody` == waterbody_name) %>% 
...
The primary sources of thermal loading contributing to temperatures exceedances along the `r waterbody_name` include increases in solar radiation loading from the disturbance or removal of near-stream vegetation, `r if(npdes.ind.n>0){paste0("point source discharges,")}` `r if(!is.na(source.char.1)){source.char.1}` and background sources (`r strip_alpha(section.model.info$"Abbreviated Reference")`). `r source.char.2` The contribution of these latter potential sources may be investigated as part of the model scenarios.
...
if(!model.extent == TRUE){
  ...
  t6x7 <- t6x7[,c(1,2,5,4)] %>% 
    dplyr::filter(`Model Location Name (Station ID)` == "PR1-5808")
