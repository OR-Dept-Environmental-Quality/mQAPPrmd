...
st_model <- station_model %>% 
  ...
  dplyr::filter(!(`Data Source` == "") & !(`Interpolated Data` == "T")) %>% 
...
`r if(model.extent == FALSE){paste(chap5x34, collapse = "\n")}else{if(NROW(t5x34)>0){knitr::kable(t5x34, format = "pandoc", padding = 2,caption = tbls(name = paste0("t5x34", waterbody_name),caption = paste0("Calibration sites and parameters usedin the existing ", waterbody_name, " Heat Source Model.")))}}`
