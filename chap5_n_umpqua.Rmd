...
```{r, label=`setup-chap5`, include=FALSE}
...
# For testing: waterbody_name <- "North Umpqua River"
# For testing: waterbody_name <- "Clearwater River"
...
model.extent <- unique(ifelse(section.model.input$`Model Waterbody` == section.model.input$`Model Extent`, TRUE, FALSE))
...
```{r, label=`t5x11`, echo=FALSE}

if(NROW(section.npdes)>0){
  
  is_are_t5x11 <- ifelse(NROW(section.npdes) == 1,"is","are")
  
  cat(paste0("There ", is_are_t5x11, " ", as.english(NROW(section.npdes)), " active individual NPDES permitted discharges on ", waterbody_name, "."))
...
```{r, label=`t5x21`, echo=FALSE}
# gsub(",.*$", "", test) to get the station in the data source

options(knitr.kable.NA = '') # hide NAs in table

t5x21 <- section.model.input %>% 
  dplyr::filter(`Site Type` %in% c("Meteorological")) %>% 
  dplyr::select(`Site Name`, Parameter, `Data Source`)

if(NROW(t5x21)>0){
  print(knitr::kable(t5x21, format = "pandoc", padding = 2,
                     caption = tbls(name = paste0("t5x21", waterbody_name),
                                    caption = paste0("Summary of available meteorological data in the ", waterbody_name," Watershed."))))
  
} else {
  
  cat(paste0("There is no available meteorological data in the ", waterbody_name," Watershed."))
...
Thermal infrared (TIR) remote sensing data were used to characterize the thermal regime of the streams and habitat quality in the basin where the TIR data were available. 

```{r, label=`t5x22`, echo=FALSE}

# for test: waterbody_name <- "Elk Creek"

if(NROW(section.tir)>0){
  
  text <- paste0("Specific uses of the TIR data in ", waterbody_name, " was documented in the report (", tir$`Abbreviated Reference`, ").")
  
} else {

  text <- paste0("There is no available TIR data in the ", waterbody_name," Watershed.")
...
Continuous temperature data were collected at a number of locations during several years. `r tbls(name=paste0("t5x23_1", waterbody_name), display="cite")` lists all the locations where continuous stream temperature data were collected and the organizations that collected that data. `r tbls(name=paste0("t5x23_2", waterbody_name), display="cite")` summarizes when data were collected at each station. Specific uses for some of this temperature data are described in the model setup and calibration sections later in this report.

```{r, label=`t5x23_1`, echo=FALSE}
...
st_model <- station_model %>% 
  ...
  dplyr::mutate(`Station Description` = ifelse(is.na(`Station Description`),`Site Name`,`Station Description`),
                Organization = ifelse(is.na(Organization),`Data Source`,Organization)) %>% 
  dplyr::select(`Station Description`,`Station ID`, Organization, Latitude, Longitude)

t5x23_1 <- rbind(st_awqms,st_model) %>% 
  dplyr::mutate(Latitude = round(Latitude,4),
                Longitude = round(Longitude,3)) %>% 
  dplyr::group_by(`Station Description`,`Station ID`, Organization, Latitude, Longitude) %>% 
  dplyr::summarise(n=n()) %>% 
  dplyr::select(-n) %>% 
...
```{r, label=`t5x23_2`, echo=FALSE}
options(knitr.kable.NA = '') # hide NAs in table

t5x23_2 <- data.sample.count %>%
  dplyr::filter(section_name %in%  waterbody_name) %>% 
  dplyr::select(Year,`Station Description`,`Station ID`,Organization,
                Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec) %>% 
  dplyr::arrange(Year,`Station Description`)
...
```{r, label=`t5x24_1`, echo=FALSE}
options(knitr.kable.NA = '') # hide NAs in table

t5x24_1 <- section.model.input %>% 
  dplyr::filter(`Parameter` %in% c("Flow")) %>% 
  dplyr::mutate(`Station Location` = ifelse(is.na(Latitude), `Station ID`, paste0(Latitude,", ", Longitude)),
                `Model_Location` = ifelse(is.na(`Model Location`), NA, paste0(`Model Location`," ", `Location Units`))) %>% 
  dplyr::arrange(desc(`Model Location`)) %>% 
  dplyr::select(`Site Name`, `Measurement Date`, `Station Location`, `Model_Location`, `Data Source`) %>% 
  dplyr::rename(`Location Description` = `Site Name`,
                `Model Location` = `Model_Location`)

if(NROW(t5x24_1)>0){
  
  cat(paste0("Flow volume data and other instream measurements were collected at ", as.english(NROW(t5x24_1)), " sites along ", waterbody_name, " (", tbls(name=paste0("t5x24_1", waterbody_name), display="cite"),")", " by Oregon DEQ. These measurements were used to develop flow mass balances for the ", waterbody_name, " temperature model."))
  
  print(knitr::kable(t5x24_1, format = "pandoc", padding = 2,
                     caption = tbls(name = paste0("t5x24_1", waterbody_name),
                                    caption = paste0("Instantaneous flow measurements in  ", waterbody_name, "."))))
  
} else {
  
  cat(paste0("Stream flow data was not used to validate the existing ", waterbody_name, " Heat Source Model."))
...
```{r, label=`t5x24_2`, echo=FALSE}

t5x24_2 <- usgs.station.tbl %>% 
  dplyr::filter(section_name %in%  waterbody_name) %>% 
  dplyr::select(-c(HUC10,section_name,n))

if(NROW(t5x24_2)>0){
  
  is_are_t5x24_2 <- ifelse(NROW(t5x24_2) == 1,"is","are")
  
  cat(paste0("There ", is_are_t5x24_2, " ", as.english(NROW(t5x24_2)), " USGS flow gaging stations on ", waterbody_name, " (",  tbls(name=paste0("t5x24_2", waterbody_name), display="cite"), ")."))

  print(knitr::kable(t5x24_2, format = "pandoc", padding = 2,
                     caption = tbls(name = paste0("t5x24_2", waterbody_name),
...
```{r, label=`t5x25`, echo=FALSE}

t5x25_1 <- section.model.input %>% 
  ...
  dplyr::mutate(`Model_Location` = ifelse(is.na(`Model Location`), NA, paste0(`Model Location`," ", `Location Units`)),
                `Permit Type and Description` = NA,
                `Stream River Mile` = NA) %>% 
  dplyr::arrange(desc(`Model Location`)) %>% 
  dplyr::select(`Site Name`, `Permit Type and Description`, `Stream River Mile`, `Model_Location`) %>% 
...
t5x25 <- section.npdes %>%
  dplyr::mutate(`Facility Name and Number` = paste0(`Common Name`," (", `WQ File Nbr`,")"),
                ...
                `Model Location` = NA) %>% 
  dplyr::arrange(desc(`River Mile`)) %>% 
  dplyr::select(`Facility Name and Number`, `Permit Type and Description`, `Stream River Mile`, `Model Location`) %>% 
  rbind(t5x25_1)
...
```{r, label=`t5x26`, echo=FALSE}
options(knitr.kable.NA = '') # hide NAs in table

t5x26 <- section.model.input %>% 
...
if(NROW(t5x26)>0){
  ...
  cat(paste0("Effective shade data in ", tbls(name=paste0("t5x26", waterbody_name), display="cite"), " were collected to assist model calibration and simulation of incoming solar radiation. All effective shade measurements were collected with a Solar Pathfinder and correspond to solar paths observed during the months of July/August."))
  
  print(knitr::kable(t5x26, format = "pandoc", padding = 2,
                     caption = tbls(name = paste0("t5x26", waterbody_name),
...
```{r, label=`t5x34`, include=FALSE}
options(knitr.kable.NA = '') # hide NAs in table

if(model.extent == FALSE){
  
  chap5x34 <- NULL
  
  for(j in unique(sort(section.model.input$`Model Extent`))){
    
    model_extent <- j
    chap5x34 <- c(chap5x34, knitr::knit_child(input = "chap5x34_n_umpqua.Rmd", envir = globalenv()))
...
`r if(model.extent == FALSE){paste(chap5x34, collapse = "\n")}else{if(NROW(t5x34)>0){knitr::kable(t5x34, format = "pandoc", padding = 2,caption = tbls(name = paste0("t5x34", waterbody_name),caption = paste0("Calibration sites and parameters used in the existing ", waterbody_name, " Heat Source Model.")))}}`
...
if(model.extent == FALSE){
  ...
  for(j in unique(sort(section.model.input$`Model Extent`))){
    ...
    model_extent <- j
    chap5x35 <- c(chap5x35, knitr::knit_child(input = "chap5x35_n_umpqua.Rmd", envir = globalenv()))
  ...
  t5x35 <- section.model.input %>% 
    dplyr::filter(!`Site Type` %in%  c("Calibration Site")) %>% 
    dplyr::mutate(`Model_Location` = ifelse(is.na(`Model Location`), NA, paste0(`Model Location`," ", `Location Units`))) %>% 
    dplyr::arrange(Parameter,desc(`Model Location`)) %>% 
    dplyr::select(`Site Name`, `Station ID`, `Model_Location`, `Parameter`, `Data Source`) %>%
    dplyr::rename(`Station Description` = `Site Name`,
...
`r if(model.extent == FALSE){paste(chap5x35, collapse = "\n")}else{if(NROW(t5x35)>0){knitr::kable(t5x35, format = "pandoc", padding = 2,caption = tbls(name = paste0("t5x35", waterbody_name),caption = paste0("Boundary condition inputs to the existing ", waterbody_name, " Heat Source Model.")))}}`
