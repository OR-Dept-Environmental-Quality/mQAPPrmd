...
Flow volume data and other instream measurements were collected at several sites along `r waterbody_name` (`r tbls(name=paste0("t5x23", waterbody_name), display="cite")`) by Oregon DEQ. These measurements were used to develop flow mass balances for the `r waterbody_name` temperature model.

```{r t5x23, echo=FALSE, results='asis'}

t5x23 <- section.model.input %>% 
...
if(NROW(t5x23)>0){
  print(knitr::kable(t5x23, format = "pandoc", padding = 2,
                     caption = tbls(name = paste0("t5x23", waterbody_name),
