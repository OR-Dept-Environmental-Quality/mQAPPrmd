...
section.model.info <- model.info %>% 
  dplyr::filter(`Section Name` %in%  waterbody_name)
...
Continuous temperature data were collected at a number of locations during several years. `r tbls(name="t5x22_1", display="cite")` lists all the locations where continuous stream temperature data were collected and the organizations that collected that data. `r tbls(name="t5x22_2", display="cite")` summarizes when data were collected at each station. Specific uses for some of this temperature data are described in the model setup and calibration sections later in this report.
...
t5x22_2 <- data.sample.count %>%
  dplyr::filter(section_name %in%  waterbody_name) %>% 
  dplyr::select(year,`Station ID`,`Station Description`,Org_Names,
...
There are no USGS flow gaging stations on `r waterbody_name`. **[THIS WILL BE CODED FOR IF TABLE ELSE NO DATA]**
...
t5x35 <- section.model.input %>% 
  dplyr::filter(!`Input Type` %in%  c("Calibration Site", "Instantaneous")) %>% 
...
* The data source and quality for the following boundary condition inputs: Spring at stream kilometer 13.2, Wolverine Creek, Brouse Creek or Hipower Creek. **[UPDATE FOR EACH STREAM]**
