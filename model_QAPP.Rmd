...
---
title: "`r paste0("Modeling Quality Assurance Project Plan for the ", subbasin, " Temperature Total
  Maximum Daily Load")`"
---
...
npdes <- readxl::read_xlsx("T:/Temperature_TMDL_Revisions/models/NPDES_communication_list.xlsx",
...
# dta.sample.count will be used in chap5 5.1.2.2
data.sample.count <- data %>% 
  ...
  dplyr::summarize(Org_Names=paste0(unique(Org_Name), collapse=" ,"),
                   n=n()) %>%
  dplyr::ungroup() %>%
  tidyr::pivot_wider(names_from = month, values_from=n) %>%
  dplyr::left_join(station_awqms[,c("MLocID","StationDes","GNIS_Name", "MonLocType",
                                    "Reachcode", "Measure",
                                    "Lat_DD", "Long_DD", "AU_ID",
                                    "HUC8_Name", "HUC10_Name","HUC12_Name",
                                    "HUC8","HUC10","HUC12","section_name")], by="MLocID")
