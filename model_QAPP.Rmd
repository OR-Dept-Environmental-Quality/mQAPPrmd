...
if(NROW(model.input[which(model.input$`Model Location Type` == "Meteorological"),])>0){
  ...
  tables <- data.frame(tbl_name = c("t51_1","t51_2","t51_3","t51_4","t51_5","t51_6"),
                       tbl_rows = c(NROW(t51_1),NROW(t51_2),NROW(t51_3),NROW(t51_4),NROW(t51_5),NROW(t51_6))) %>% 
  ...
  tables <- data.frame(tbl_name = c("t51_2","t51_3","t51_4","t51_5","t51_6"),
                       tbl_rows = c(NROW(t51_2),NROW(t51_3),NROW(t51_4),NROW(t51_5),NROW(t51_6))) %>% 
    dplyr::filter(tbl_rows != "0")
...
Meteorological data, including air temperature, cloudiness, relative humidity, and wind speed, are available from NOAA National Climatic Data Center (NCDC), National Interagency Fire Center (NIFC) Remote Automatic Weather Stations (RAWS), Bureau of Reclamation Cooperative Agricultural Weather Network (AgriMet) and Automated Hydrologic and Meteorologic monitoring stations Network (Hydromet), and University of Utah MesoWest database. `r ifelse(NROW(model.input[which(model.input$"Model Location Type" == "Meteorological"),])>0,tbls(name = "t51_1", display="cite"),tbls(name = "t51_2", display="cite"))` - Table `r t51_num` list available meteorological data from these sources in the `r qapp_project_area`.

```{r, label=`t51_tbls`, echo=FALSE}

if(NROW(model.input[which(model.input$`Model Location Type` == "Meteorological"),])>0){
  
  if(NROW(t51_1)>0){
    print(knitr::kable(t51_1, format = "pandoc", padding = 2,
                       caption = tbls(name = "t51_1",
                                      caption = paste0("Meteorologcial stations and data available in public databases and DEQ files in the ", qapp_project_area, "."))))
  }
...
if(NROW(t51_2)>0){
  print(knitr::kable(t51_2, format = "pandoc", padding = 2,
                     caption = tbls(name = "t51_2",
                                    caption = paste0("Meteorologcial stations and data available in the National Climatic Data Center (NCDC) database in the ", qapp_project_area, "."))))
}

if(NROW(t51_3)>0){
  print(knitr::kable(t51_3, format = "pandoc", padding = 2,
                     caption = tbls(name = "t51_3",
                                    caption = paste0("Meteorologcial stations and data, including ", knitr::combine_words(unique(sort(tolower(raws.station.tbl$Parameter)))), ", available in the Remote Automatic Weather Station (RAWS) database in the ", qapp_project_area, "."))))
}

if(NROW(t51_4)>0){
  print(knitr::kable(t51_4, format = "pandoc", padding = 2,
                     caption = tbls(name = "t51_4",
                                    caption = paste0("Meteorologcial stations and data, including air temperature, precipitation, relative humidity and wind, available in the USBL AgriMet database in the ", qapp_project_area, "."))))
}

if(NROW(t51_5)>0){
  print(knitr::kable(t51_5, format = "pandoc", padding = 2,
                     caption = tbls(name = "t51_5",
                                    caption = paste0("Meteorologcial stations and data available in the USBL Hydromet database in the ", qapp_project_area, "."))))
}

if(NROW(t51_6)>0){
  print(knitr::kable(t51_6, format = "pandoc", padding = 2,
                     caption = tbls(name = "t51_6",
                                    caption = paste0("Meteorologcial stations and data, including air temperature, precipitation, relative humidity, wind speed and wind direction, available in the MesoWest database in the ", qapp_project_area, "."))))
}
...
`r tbls(name = "t54", display="cite")` lists the stations for flow volume data and other instream measurements that were available in USGS, Oregon DEQ, and other public databases.
...
t54 <- rbind(t54_1,t54_2,t54_3) %>% 
  dplyr::arrange(Station)

if(NROW(t54)>0){
  
  print(knitr::kable(t54, format = "pandoc", padding = 2,
                     caption = tbls(name = "t54",
                                    caption = paste0("Flow measurements in  ", qapp_project_area, "."))))
...
`r tbls(name = "t55_1", display="cite")` identifies all the active individual NPDES permittees in the `r qapp_project_area`. Many of these permittees submit Discharge Monitoring Reports (DMRs) as a condition of their permit. Depending on the monitoring requirements in the permit, some permittees are required to report effluent temperature and effluent flow rates in the DMR. The frequency and type of reporting varies by permit and permit type. Some permits only require monthly, weekly, or daily grab samples while others require summary statistics such as daily maximum, daily mean, or seven day average daily maximum. When possible, DEQ will utilize any continuous effluent data that has been provided to DEQ. When continuous data is not available, DMR data will be utilized to characterize point source discharges. `r tbls(name = "t55_2", display="cite")` lists all the active general NPDES permitting types in the `r qapp_project_area`.
...
if(NROW(section.npdes.ind)>0){
  ...
  print(knitr::kable(section.npdes.ind, format = "pandoc", padding = 2,
                     caption = tbls(name = "t55_1",
...
if(NROW(section.npdes.gen)>0){
  ...
  print(knitr::kable(section.npdes.gen, format = "pandoc", padding = 2,
                     caption = tbls(name = "t55_2",
