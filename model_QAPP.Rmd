...
t53_1 <- rbind(st_awqms,st_model) %>% 
  ...
  dplyr::select(Station,`Station ID`,`Latitude/Longitude`, Organization) %>% 
...
t53_2 <- data.sample.count %>%
  dplyr::filter(!`Station ID` == "TIR") %>% 
  dplyr::mutate(`Station (Station ID)` = ifelse(`Station ID` == "No Station ID or unknown" | is.na(`Station ID`),
                                               StationDes,
                                               paste0(StationDes, " (", `Station ID`, ")"))) %>% 
  dplyr::select(Year, `Station (Station ID)`, Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec) %>% 
  dplyr::arrange(Year, `Station (Station ID)`)
...
if(mod.check == "11"){
  ...
  hs67.tbl <- knitr::kable(hs67, format = "pandoc", padding = 2,
                           caption = tbls(name = "hs67", 
                                          caption = paste0("Summary of model inputs and input categories required for Heat Source version 6 and 7.")))
  ...
  if(mod.check == "21"){
    ...
    hs68.tbl <- knitr::kable(hs68, format = "pandoc", padding = 2,
                             caption = tbls(name = "hs68", 
                                            caption = paste0("Summary of model inputs and input categories required for Heat Source version 6 and 8.")))
...
#
# Note: SHADOW model need to be filtered out from model.info.
chap6 <- NULL
