...
Effective shade is the percent of potential daily solar radiation flux that is blocked by vegetation and topography. DEQ and/or partner agency staff used an instrument called a Solar Pathfinder to collect effective shade measurements in the field. The effective shade measurement methods and quality control procedures used are outlined in the Water Quality Monitoring Technical Guide Book (OWEB, 1999). `r if(NROW(t57)>0){paste0(tbls(name = "t57", display="cite")," lists the locations where effective shade measurements were collected and the effective shade value for July 2001.")} else {paste0("There are no effective shade data available to DEQ or within publically accessible databases in the ", qapp_project_area, ".")}`
...
Not listed in `r tbls(name = "tmdl.mod.a", display="cite")` are existing SHADOW models for various streams in the Sucker Creek Watershed, a Heat Source temperature model for Sucker Creek, and Heat Source shade models for Cheney Creek, Thompson Creek, Slate Creek, and Waters Creek in the Applegate Subbasin. These models were documented in their respective TMDLs but unfortunately have gone missing from DEQ’s files. DEQ does not plan to replace these models with new ones. Instead DEQ will utilize a “Shave Curve” approach as described in DEQ 2004 and DEQ 2008.
...
* Updating the Applegate River model to characterize the current discharge from Hidden Valley High School STP.        
