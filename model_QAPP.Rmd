...
Erin Costello, Yuan Grund, and Ryan Michie
...
**[Update:]**The existing temperature TMDL addresses temperature on streams with both point and nonpoint sources of thermal loads. 
...
# add 353036 from the worksheet to the RAWS table
sp.raws <- data.frame("orOGAD","GRANDAD", "43.4158/122.577","USFS")
names(sp.raws) <- c("Station ID","Station","Latitude/Longitude","Agency" )
...
if(NROW(model.input[which(model.input$`Model Location Type` == "Meteorological"),])>0){
  ...
  t51.sp <- t51 %>% 
    ...
    dplyr::filter(!Source == "353036, RAWS") # add this site to the RAWS table
...
if(NROW(model.input[which(model.input$`Model Location Type` == "Meteorological"),])>0){
  ...
  tables.met <- data.frame(tbl_name = c("t51_ncei","t51_raws","t51_agrimet","t51_hydromet","t51_mw"),
...
All available continuous stream temperature data were retrieved from DEQ’s Ambient Water Quality Monitoring System (AWQMS). `r tbls(name = "t53_1", display="cite")` lists all the locations where continuous stream temperature data were collected and the organizations that collected that data. Appendix A summarizes when data were collected at each station. The location of these stations is shown in the HTML interactive map that accompanies this QAPP and referenced in Appendix C.
...
# North Umpqua instantaneous flow
t54_nu <- readxl::read_xlsx("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/tables_NorthUmpqua.xlsx",sheet = "NorthUmpqua_InstFlow") %>% 
  dplyr::mutate(Time = format(Time,"%H:%M")) %>% 
  dplyr::select(Station,Date,Time,`Flow (cfs)`,`Latitude/Longitude`) 

t54_nu_tbl <- knitr::kable(t54_nu, format = "pandoc", padding = 2, row.names = NA,
                           caption = tbls(name = "t54_nu",
                                          caption = paste0("Instantaneous flow volume measurements in the ", qapp_project_area, ".")))


```

`r if(NROW(t54.tbl.num) == 1){paste0(tbls(name = t54.tbl.num$table, display="cite"), " lists")} else {paste0(tbls(name = t54.tbl.num$table[1], display="cite"), " through ", tbls(name = t54.tbl.num$table[length(t54.tbl.num$table)], display="cite"), " list")}` the stations for flow volume data and other instream measurements that were available in USGS, DEQ, and other public databases. Appendix B summarizes when data were collected at each of the stations with continuous flow measurements. The location of these stations is shown in the HTML interactive map that accompanies this QAPP and referenced in Appendix C. `r tbls(name = "t54_nu", display="cite")` lists the locations where instantaneous flow volume measurements made by DEQ were available.
...
section.npdes.gen.1 <- npdes.gen.pro.area %>% 
  dplyr::filter(`Project Area` %in%  qapp_project_area) %>%
  dplyr::filter(PermitType %in% c("GEN01","GEN03","GEN04","GEN05")) %>% 
...
if(NROW(section.npdes.gen.1)>0){
  ...
  t55_gen1_tbl <- knitr::kable(section.npdes.gen.1, format = "pandoc", padding = 2,
                              caption = tbls(name = "t55_gen1",
                                             caption = paste0("Summary of any current general NPDES GEN01, GEN03, GEN04 and GEN05 permitted discharges in the ", qapp_project_area, ".")))
...
if(NROW(section.npdes.gen.2)>0){
  ...
  t55_gen2_tbl <- knitr::kable(section.npdes.gen.2, format = "pandoc", padding = 2,
                              caption = tbls(name = "t55_gen2",
                                             caption = paste0("Summary of current number of general NPDES permit registrants, other than GEN01, GEN03, GEN04 and GEN05 registrants, in the ", qapp_project_area, ".")))
  
}

```

`r if(NROW(section.npdes.ind)>0){tbls(name = "t55_ind", display="cite")}else{paste0("[Update: No individual NPDES permits]")}` identifies all the active individual NPDES permittees in the `r qapp_project_area`. `r if(NROW(section.npdes.gen.1)>0){paste0(tbls(name = "t55_gen1", display="cite"), " lists the permittees of all the active  general NPDES GEN01, GEN03, GEN04 and GEN05 permits in the ", qapp_project_area, ". ")}`The location of these individual NPDES permittees is shown in the HTML interactive map that accompanies this QAPP and is referenced in Appendix C. Many of these permittees submit Discharge Monitoring Reports (DMRs) as a condition of their permit. Depending on the monitoring requirements in the permit, some permittees are required to report effluent temperature and effluent flow rates in the DMR. The frequency and type of reporting varies by permit and permit type. Some permits only require monthly, weekly, or daily grab samples while others require summary statistics such as daily maximum, daily mean, or seven-day average daily maximum. Where possible, DEQ will utilize any continuous effluent data that has been provided to DEQ. When continuous data is not available, DMR data will be utilized to characterize point source discharges. `r if(NROW(section.npdes.gen.2)>0){tbls(name = "t55_gen2", display="cite")}` lists all the active general NPDES permitting types `r if(NROW(section.npdes.gen.1)>0){paste0("other than GEN01, GEN03, GEN04 and GEN05 permitting types ")}`in the `r qapp_project_area`.
...
```{r, label=`t56_nu`, echo=FALSE}

t56.nu <- readxl::read_xlsx("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/tables_NorthUmpqua.xlsx",
                               sheet = "NorthUmpqua_effectiveShade") %>% 
  dplyr::mutate(site = scales::percent(site, accuracy = 1),
                `u/s 150 m` = scales::percent(`u/s 150 m`, accuracy = 1),
                `u/s 100 m` = scales::percent(`u/s 100 m`, accuracy = 1),
                `u/s 50 m` = scales::percent(`u/s 50 m`, accuracy = 1),
                `d/s 50 m` = scales::percent(`d/s 50 m`, accuracy = 1)) %>% 
  dplyr::rename(`At Location` = site,
                `Upstream 150m` = `u/s 150 m`,
                `Upstream 100m` = `u/s 100 m`,
                `Upstream 50m` = `u/s 50 m`,
                `Downstream 50m` = `d/s 50 m`) %>% 
  dplyr::select(Location,`Latitude/Longitude`,`At Location`,`Upstream 150m`,`Upstream 100m`,`Upstream 50m`,`Downstream 50m`) %>%
  dplyr::arrange(Location)

t56_nu_tbl <- knitr::kable(t56.nu, format = "pandoc", padding = 2,
                              caption = tbls(name = "t56.nu",
                                             caption = paste0("Effective shade data collected in the  ", qapp_project_area, ".")))

```

Effective shade is the percent of potential daily solar radiation flux delivered to the stream surface. DEQ used an instrument called a Solar Pathfinder to collect effective shade measurements in the field. `r if(NROW(t56.nu)>0){paste0(tbls(name = "t56.nu", display="cite")," lists the locations where effective shade measurements were collected and the effective shade value for September 2009.")} else {paste0("There are no effective shade data available to DEQ or within publically accessible databases in the ", qapp_project_area, ".")}`

`r if(NROW(t56.nu)>0){t56_nu_tbl}`
...
knitr::kable(rev, format = "pandoc", padding = 2,
