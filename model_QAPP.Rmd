...
temp_cal_sites <- model.input %>% 
  ...
  #dplyr::filter(`Calibration Parameter` == "Water Temperature") %>% 
  dplyr::filter(!is.na(Latitude)) %>% 
  dplyr::group_by(`Model Location Name`, `Model Location`, `Location Units`, `Data Source`, Latitude, Longitude) %>% 
  dplyr::summarize(`Calibration Parameter` = toString(unique(sort(`Calibration Parameter`)))) %>% 
  dplyr::ungroup() %>% 
