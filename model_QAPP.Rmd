...
t56.lw <- readxl::read_xlsx("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/tables_LowerWillamette.xlsx",
...
t56_lw_tbl <- knitr::kable(t56.lw, format = "pandoc", padding = 2,
                              caption = tbls(name = "t56.lw",
                                             caption = paste0("Effective shade data collected along Johnson Creek.")))

```

Effective shade is the percent of potential daily solar radiation flux delivered to the stream surface. DEQ used an instrument called a Solar Pathfinder to collect effective shade measurements in the field. `r if(NROW(t56.lw)>0){paste0(tbls(name = "t56.lw", display="cite")," lists the locations where effective shade measurements were collected and the effective shade value for July 2002.")} else {paste0("There are no effective shade data available to DEQ or within publically accessible databases in the ", qapp_project_area, ".")}`

`r if(NROW(t56.lw)>0){t56_lw_tbl}`
...
The setup and calibration for the Johnson Creek model was completed by DEQ and documented in the `r paste0(pro.area.tmdls, " (", knitr::combine_words(strip_alpha(unique(sort(ref[which(ref$'Project Area' == qapp_project_area),]$"Abbreviated Reference")))), ")")`. The setup and calibration of the Columbia Slough model was completed by Portland State University and documented in Wells and Berger, 1995; Berger 2003; Berger and Wells, 2005; and DEQ, 2006. Adjustments to the existing Johnson Creek and Columbia Slough calibrated models are unlikely to occur as part of this project. However, if it is determined that the model calibration needs to be updated, the model inputs that are expected to be modified are described in section 6.1. An improved model fit is defined as an improvement to the model fit statistics described in Chapter 7.
