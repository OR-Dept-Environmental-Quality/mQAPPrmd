...
The CE-QUAL-W2 model (Cole and Wells, 2000) meets the model framework needs for stratified waterbodies with flow control structures, and was used to model the Columbia Slough for the development of the 2006 Willamette Basin TMDL. Although this model exists and meets model framework needs, significant changes to flow and channel morphology have occurred in the Columbia Slough since the model was calibrated. The model needs to be recalibrated with updates to bathymetry and flow control structure inputs in order to simulate current water temperature conditions in the system. A summary of the recommended model updates is included in Appendix C. Due to resource constraints and the rigorous project schedule, the existing Columbia Slough CE-QUAL-W2 model will not be used for TMDL development at this time.
...
The setup and calibration for the models listed in `r tbls(name = "tmdl.mod.a", display="cite")` was completed by DEQ and Portland State University and documented in the `r paste0(pro.area.tmdls, " (", knitr::combine_words(strip_alpha(unique(sort(ref[which(ref$'Project Area' == qapp_project_area),]$"Abbreviated Reference")))), ")")`. Adjustments to the existing calibrated models are unlikely to occur as part of this project. However, if it is determined that the model calibration needs to be updated, the model inputs that are expected to be modified are described in section 6.1. An improved model fit is defined as an improvement to the model fit statistics described in Chapter 7.
...
* Modifying the Portland International Airport and Arclin point source discharges to the Columbia Slough to assess the effects of cumulative warming.        
