...
Stream temperatures for the 2005 Sandy River Basin TMDL (DEQ, 2005) were simulated using the computer model Heat Source Version 6. New models are proposed using Heat Source Version 8. The models extents include most of the main rivers and their larger tributaries that contain or influence primary fish habitat. Site-specific load allocations have been developed for the streams that were simulated. Other streams are assigned generalized load allocations based on potential vegetation and effective shade.
...
st_owrd <- station.owrd.temp %>% 
  dplyr::mutate(Organization = "OWRD") %>% 
  dplyr::rename(Latitude = Lat,
                Longitude = Long) %>% 
  dplyr::select(Station,`Station ID`, Organization, Latitude, Longitude)

t53_1 <- rbind(st_awqms,st_model,st_owrd) %>% 
...
t53_1_usgs_or <- t53_1 %>% 
  dplyr::filter(Organization == "USGS-OR") %>% 
...
# Sandy instantaneous flow
t54_sandy <- readxl::read_xlsx("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/tables.xlsx",sheet = "sandy_instantaneous_flow") %>% 
  dplyr::left_join(df.stations, by = c("Station ID"="MLocID")) %>% 
  dplyr::mutate(`Latitude/Longitude` = ifelse(is.na(Lat_DD), NA, paste0(round(Lat_DD,4), "/",round(Long_DD,3)))) %>% 
  dplyr::select(`Station ID`, Station, Date, `Flow (cfs)`, `Latitude/Longitude`) 

t54_sandy_tbl <- knitr::kable(t54_sandy, format = "pandoc", padding = 2,
                              caption = tbls(name = "t54_sandy",
                                             caption = paste0("Instantaneous flow measurements made by DEQ in the ", qapp_project_area, ".")))


t54.tbl.num <- data.frame(table = c("t54_usgs","t54_owrd","t54_hydromet","t54_sp","t54_sandy"),
                          `nrow` = c(NROW(t54_usgs),NROW(t54_owrd),NROW(t54_hydromet),NROW(t54_sp),NROW(t54_sandy))) %>% 
  dplyr::filter(!`nrow` == 0)
...
`r if(NROW(t54_sandy)>0){t54_sandy_tbl}`
...
```{r, label=`t56_sandy`, echo=FALSE}

t56_sandy <- readxl::read_xlsx("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/tables.xlsx",sheet = "sandy_effective_shade") %>% 
  #dplyr::mutate(`Effective Shade` = ifelse((`Effective Shade` %in% c("5-30%","15-40%","88-94%","88-100%","95-100%")),as.character(`Effective Shade`),scales::percent(as.numeric(`Effective Shade`)))) %>% 
  dplyr::mutate(`Effective Shade` = ifelse((`Effective Shade` %in% c("5-30%","15-40%","88-94%","88-100%","95-100%")),as.character(`Effective Shade`),paste0(round(as.numeric(`Effective Shade`)*100),"%"))) %>% 
  dplyr::left_join(df.stations, by = c("Station ID"="MLocID")) %>% 
  dplyr::mutate(`Latitude/Longitude` = ifelse(is.na(Lat_DD), NA, paste0(round(Lat_DD,4), "/",round(Long_DD,3)))) %>% 
  dplyr::mutate(`Latitude/Longitude` = ifelse(`Station ID` == "26397-ORDEQ", "45.5535/-122.387", `Latitude/Longitude`)) %>% 
  dplyr::mutate(`Latitude/Longitude` = ifelse(Station == "Sandy R. @ Troutdale STP", "45.4982/-122.01", `Latitude/Longitude`)) %>% 
  dplyr::select(`Station ID`,Station,`Latitude/Longitude`,`Effective Shade`,`Data Source`) 

t56_sandy_tbl <- knitr::kable(t56_sandy, format = "pandoc", padding = 2,
                              caption = tbls(name = "t56_sandy",
                                             caption = paste0("Effective shade data collected in the  ", qapp_project_area, ".")))

```

Effective shade is the percent of potential daily solar radiation flux delivered to the stream surface. DEQ and/or partner agency staff used an instrument called a Solar Pathfinder to collect effective shade measurements in the field. `r tbls(name = "t56_sandy", display="cite")` lists the stations for effective shade measurements that were available in public and DEQ databases.
...
Waterbodies where model development was initiated for the 2005 Sandy River Basin TMDL (DEQ, 2005) are listed in `r tbls(name = "tmdl.mod.a", display="cite")`. `r if(NROW(tmdl.mod.b)>0){paste0("The waterbodies listed in ", tbls(name = "tmdl.mod.b", display="cite")," will have new models developed.")}`
...
The models for the Little Sandy River and Zigzag River were originally developed for the Sandy River Basin TMDL (DEQ, 2005) will also be used to support the updated TMDL. The models for the Sandy River, Bull Run River, and Salmon River (`r tbls(name = "tmdl.mod.b", display="cite")`) are new and will supplement the models that were developed for the 2005 TMDL.
...
It is not anticipated that any additional uncertainty or sensitivity analyses will be performed on the calibrated models originally developed for the 2005 Sandy Temperature TMDL (DEQ 2005). 
...
*	Removal of all point sources from the Sandy River model.    
...
```{r, label=`t1011`, echo=FALSE}

t1011_sandy <- readxl::read_xlsx("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/tables.xlsx",sheet = "sandy_nodam")

t1011_sandy_tbl <- knitr::kable(t1011_sandy, format = "pandoc", padding = 2,
                              caption = tbls(name = "t1011_sandy",
                                             caption = paste0("Boundary condition and tributary inputs to the no dam scenario on the Bull Run River.")))

```

This scenario is specific to the Bull Run River and Sandy River. This scenario estimates the stream temperatures on the Bull Run River without Bull Run Dam and Reservoir Number One and Bull Run Dam and Reservoir Number Two. The model extent will be from the Bull Run River mouth to just upstream of Bull Run Reservoir Number Two at USGS Gage 14138850. No changes are proposed on the Sandy River model except for setting the Bull Run River input to the no dam temperature outputs. The stream temperature warming or cooling caused by the dams is evaluated by comparing this results of this scenario to the current condition model run and data collected at the various temperature monitoring locations downstream of Bull Run Reservoir Number Two including the following:        

* PWB_BR_DNSTM_PP: Bull Run River approximately 600 feet downstream of Bull Run Dam 2 spillway plunge pool        
* USGS Gage 14140000: Bull Run River Near Bull Run (River Only), OR       
* USGS Gage 14140020: Bull Run R at Larson's Bridge, Near Bull Run, OR        
* PWB_BR_DODGE: Bull Run River approximately 500 feet upstream of Sandy River confluence        

This scenario will also be used to estimate the background rate of warming that occurs within the reaches where the reservoirs and dams are currently located. This warming rate may be used, in conjunction with data collected at the various gages in the Bull Run and Little Sandy Rivers to develop management targets for the Bull Run Reservoirs similar to the current temperature target surrogate measures in the 2006 Sandy TMDL (DEQ, 2005).

To complete this scenario, DEQ anticipates extending the 2016 current condition Heat Source model upstream through the reservoir reaches but may also consider using a CE-QUAL-W2 model developed by Portland State University for The City of Portland (Annear, et al., 1999, City of Portland, 2004). The CE-QUAL-W2 model was developed as a no dam scenario for the 2001 model year. The City of Portland has updated this model over time and currently uses it for management objectives. Use of this model at minimum would require updates to the boundary conditions and meteorological inputs. At the current time this model is not available to DEQ. Should this model become available DEQ will review the model to make a determination on its use considering the current model setup, data availability, and the amount of time and work required to update the model.

Development of a riverine model in place of Bull Run Reservoir Number One and Bull Run Reservoir Number will require information about channel position, width, slope, substrate, and manning’s roughness coefficients. Existing bathymetry and any pre dam channel measurements, if available, will be used to develop the model setup. `r tbls(name = "t1011_sandy", display="cite")` summarizes the various data sources to be used for the boundary conditions, tributary, and meteorological inputs for the no dam scenario. All other inputs downstream of Bull Run Dam Number 1 will be the same as the current condition model.    
...
other.ref <- data.frame(reference = c(
  ...
  'City of Portland. 2004. "Bull Run River Temperature Evaluation."'
))
