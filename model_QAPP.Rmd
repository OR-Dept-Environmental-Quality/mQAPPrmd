...
# for test: qapp_project_area = "Upper Rogue Subbasin"
# for test: metsite <- "Medford Airport, National Climatic Data Center"

for(metsite in unique(sort(model.input$`Data Source`))){
...
t51_1 <- model.input %>% 
  ...
  dplyr::summarise(Parameter = toString(Parameter))

if(NROW(t51_1)>0){
  print(knitr::kable(t51_1, format = "pandoc", padding = 2,
                     caption = tbls(name = paste0("t51_1",metsite),
                                    caption = paste0("Meteorological data avialable at ", metsite, " in ", qapp_project_area, "."))))
}
...
t51_2 <- ncdc.station.tbl %>% 
  dplyr::mutate(Location = paste0(name," (", station.id,")"),
...
if(NROW(t51_2)>0){
  print(knitr::kable(t51_2, format = "pandoc", padding = 2,
                     caption = tbls(name = "t51_2",
                                    caption = paste0("Meteorological data of ", qapp_project_area, " available in NOAA National Climatic Data Center database."))))
}
...
if(NROW(t51_3)>0){
  print(knitr::kable(t51_3, format = "pandoc", padding = 2,
                     caption = tbls(name = "t51_3",
                                    caption = paste0("Meteorological data of ", qapp_project_area, " available in USBL Cooperative Agricultural Weather Network database."))))
}
...
if(NROW(t51_4)>0){
  print(knitr::kable(t51_4, format = "pandoc", padding = 2,
                     caption = tbls(name = "t51_4",
                                    caption = paste0("Meteorological data of ", qapp_project_area, " available in Mesowest database."))))
}
...
All available continuous stream temperature data were retrieved from DEQ’s Ambient Water Quality Monitoring System (AWQMS). Table X to Table Y list all the locations where continuous stream temperature data were collected and the organizations that collected that data. Table Y to Table Z summarizes when data were collected at each station.

```{r, label=`t53`, echo=FALSE}

for(waterbody_name in unique(sort(station_awqms$section_name))){
  
  # test: waterbody_name <- "Canton Creek"
  ...
  t53_1 <- rbind(st_awqms,st_model) %>% 
    ...
    dplyr::mutate(`Latitude/Longitude` = ifelse(is.na(Latitude), NA, paste0(round(Latitude,4), ", ",round(Longitude,3)))) %>% 
    dplyr::group_by(Location,`Latitude/Longitude`, Organization) %>% 
    dplyr::summarise(n=n()) %>% 
    dplyr::ungroup() %>% 
    dplyr::select(-n) %>% 
    dplyr::arrange(Location)
  ...
  t53_2 <- data.sample.count %>%
    ...
    dplyr::mutate(Location = ifelse(`Station ID` == "No Station ID or unknown" | is.na(`Station ID`),
                                    `Station Description`,
                                    paste0(`Station Description`, " (", `Station ID`, ")"))) %>% 
    dplyr::select(Year, Location,Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec) %>% 
    dplyr::arrange(Year, Location)
  
  if(NROW(t53_1)>0){
    print(knitr::kable(t53_1, format = "pandoc", padding = 2,
                       caption = tbls(name = paste0("t53_1", waterbody_name),
                                      caption = paste0("Continuous temperature monitoring locations in the ", waterbody_name, " Watershed currently available in public databases and DEQ files."))))
  }
  
  if(NROW(t53_2)>0){
    print(knitr::kable(t53_2, format = "pandoc", padding = 2,
                       caption = tbls(name = paste0("t53_2", waterbody_name),
...
Table X - Table Y list flow volume data and other instream measurements that were collected by Oregon DEQ.
...
for(waterbody_name in unique(sort(station_awqms$section_name))){
  ...
  if(NROW(t54_1)>0){
    ...
    print(knitr::kable(t54_1, format = "pandoc", padding = 2,
                       caption = tbls(name = paste0("t54_1", waterbody_name),
                                      caption = paste0("Instantaneous flow measurements in  ", waterbody_name, "watershed."))))
...
The USGS maintains a number of streamflow gages in the North Umpqua Subbasin. Table X - Table Y list the USGS flow gaging stations.
...
for(waterbody_name in unique(sort(station_awqms$section_name))){
  ...
  if(NROW(t54_2)>0){
    ...
    print(knitr::kable(t54_2, format = "pandoc", padding = 2,
                       caption = tbls(name = paste0("t54_2", waterbody_name),
...
```{r, label=`t55`, echo=FALSE}
  ...
  if(NROW(t55)>0){
    ...
    print(knitr::kable(t55, format = "pandoc", padding = 2,
                       caption = tbls(name = paste0("t55", waterbody_name),
...
for(waterbody_name in unique(sort(station_awqms$section_name))){
  ...
  if(NROW(t56)>0){
    ...
    print(knitr::kable(t56, format = "pandoc", padding = 2,
                       caption = tbls(name = paste0("t56", waterbody_name),
                                      caption = paste0("Effective shade data collected along  ", waterbody_name, "watershed."))))
