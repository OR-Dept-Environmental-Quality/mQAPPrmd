...
---
title: "`r paste0("Modeling Quality Assurance Project Plan for the ", qapp_project_area, " Temperature Total
  Maximum Daily Load")`"
---
...
cal.input <- readxl::read_xlsx(paste0(data.dir, "Model_Setup_Info.xlsx"),
                               sheet = "Calibration Inputs")
  
cal.model <- readxl::read_xlsx(paste0(data.dir, "Model_Setup_Info.xlsx"),
                               sheet = "Calibration Model Info")

npdes <- readxl::read_xlsx(paste0(data.dir, "NPDES_communication_list.xlsx"),
                               sheet = "Individual_NDPES")
...
# data.sample.count will be used in chap5 5.x.2.2
...
# USGS Flow data ----
# UPDATE REQUIRED:
subbasin_num <- "17100301"

usgs.stations <- whatNWISdata(stateCd="OR",
                              parameterCd = "00060") %>%  # 00060	= Discharge [ft3/s]
  dplyr::filter(huc_cd %in% subbasin_num) %>%
  dplyr::filter(site_tp_cd %in% c("ST")) %>% # ST = Stream
  dplyr::filter(data_type_cd %in% c("dv")) %>%  # dv = Daily
...
usgs.station.subbasins <-  sf::st_join(x = usgs.stations,
                                       y = qapp_project_area_huc12,
                                       join = st_intersects,
                                       left = TRUE)

# ggplot2::ggplot(usgs.station.subbasins) + geom_sf()

st_geometry(usgs.station.subbasins) <- NULL

usgs.station.tbl <- usgs.station.subbasins %>% 
  dplyr::select(station_nm, agency_cd, dec_lat_va, dec_long_va, begin_date, end_date, HUC_10) %>% 
  dplyr::rename(`Station Name`= station_nm,
                Agency = agency_cd,
                Latitude = dec_lat_va,
                ...
                HUC10 = HUC_10) %>% 
  dplyr::left_join(lookup_section_name, by="HUC10") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " R ", " RIVER ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " @ ", " AT ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " & ", " AND ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " CK ", " CREEK ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " CR ", " CREEK ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " NR ", " NEAR ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, "N\\.", "NORTH ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, "N ", "NORTH ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " ABV ", " ABOVE ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " BLW ", " BELOW ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " P ", " POWER ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, " CA ", " CANAL ") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, "OREG", "OR") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, "\\.", "") %>% 
  dplyr::mutate_at("Station Name", str_replace_all, "T FLS", "TOKETEE FALLS")
...
Error statistics were calculated for both the spatial (TIR) and temporal (hourly instream measurements) temperatures.  Table 64 contains the error statistics for each stream simulated. 

**[Compare]**
Model fit error statistics were calculated for the calibrated model for both hourly temperatures and 7DADM stream temperatures using Equation A6 - Equation A9.
...
roles <- readxl::read_xlsx("T:/Temperature_TMDL_Revisions/models/roles.xlsx",
                               sheet = "roles")
