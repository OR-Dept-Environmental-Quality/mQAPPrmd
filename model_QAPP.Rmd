...
if(NROW(tcat5)>0){
  ...
  tcat5_tbl <- knitr::kable(tcat5, format = "pandoc", padding = 2,
                        caption = tbls(name = "tcat5",
                                       caption = paste0(qapp_project_area_wms," assessment units that are classified as water quality limited category 5 for temperature based on the Section 303(d) 2018/2020 Integrated Report.")))
  ...
  if(NROW(tcat4a)>0){
    ...
    tcat4a_tbl <- knitr::kable(tcat4a, format = "pandoc", padding = 2,
                           caption = tbls(name = "tcat4a",
                                          caption = paste0(qapp_project_area_wms," assessment units that are classified as water quality limited category 4A for temperature based on the Section 303(d) 2018/2020 Integrated Report.")))
...
The Clean Water Act requires TMDLs be developed for waterbodies that do not meet water quality standards and are listed as water quality impaired on the State’s 303(d) list. The `r paste0(qapp_project_area_wms, if(str_sub(qapp_project_area,-1,-1) == "s"){paste0(" include")}else{paste0(" includes")})` several waterbodies listed on the Oregon 2018/2020 Section 303(d) Category 5 list as water quality limited for temperature (`r tbls(name = "tcat5", display="cite")`). `r if(length(unique(model.info$'TMDL Document'))==1){paste0("A TMDL was previously developed for the ", qapp_project_area_wms, " (", knitr::combine_words(strip_alpha(unique(sort(ref[which(ref$'Project Area' == qapp_project_area),]$"Abbreviated Reference"))),sep = "; "), ") but it must be replaced due to recent litigation.")}else{paste0("TMDLs were previously developed for the ",qapp_project_area_wms, " (", knitr::combine_words(strip_alpha(unique(sort(ref[which(ref$'Project Area' == qapp_project_area),]$"Abbreviated Reference"))),sep = "; "), ") but they must be replaced due to recent litigation.")}`
...
**[Update: Only the new models will be discussed in this QAPP.]** Stream temperatures will be simulated using `r if(NROW(tmdl.mod.b)>1){paste0("the computer models")} else {paste0("a computer model")}` (`r if(NROW(tmdl.mod.b)>1){knitr::combine_words(tmdl.mod.b$'Model Version')}`) for the main rivers and their larger tributaries. Simulations focus on the larger streams that contain or influence primary fish habitat. Site-specific load allocations will be developed for the streams that are simulated. Other streams are assigned generalized load allocations based on potential vegetation and effective shade.
