...
# Updated for IR2022 on 11/15/2022
tcat5 <- pro.cat.45.tbl %>%
  dplyr::filter(AU_parameter_category == "5") %>% 
  dplyr::filter(AU_parameter_category == "5") %>%
  dplyr::rename(`Assessment Unit Name` = AU_Name,
                `Assessment Unit ID` = AU_ID,
                `Use Period (Year Listed)` = `Period_(Year_Listed)`) %>% 
  dplyr::select(`Assessment Unit Name`,`Assessment Unit ID`,`Use Period (Year Listed)`) %>% 
  dplyr::arrange(`Assessment Unit Name`)
...
# Updated for IR2022 on 11/15/2022
tcat4a <- pro.cat.45.tbl %>%
  dplyr::filter(AU_parameter_category == "4A") %>%
  dplyr::rename(`Assessment Unit Name` = AU_Name,
                `Assessment Unit ID` = AU_ID,
                `Use Period (Year Listed)` = `Period_(Year_Listed)`) %>% 
  dplyr::select(`Assessment Unit Name`,`Assessment Unit ID`,`Use Period (Year Listed)`) %>% 
...
section.npdes.ind <- npdes.ind.pro.area %>% 
  ...
  dplyr::rename(`Permit Number` = `Permit Nbr`) %>% 
  dplyr::select(`Facility Name (Facility Number)`, `Latitude/Longitude`,`Permit Number`,`EPA Number`,`Permit Type and Description`, `Stream River Mile`) %>% 
...
if(NROW(section.npdes.ind)>0){
  ...
  t55_ind_tbl <- knitr::kable(section.npdes.ind[,-c(3,4)], format = "pandoc", padding = 2,
...
section.npdes.gen.1 <- npdes.gen.pro.area %>% 
  ...
  dplyr::rename(`Permit Number` = PermitNbr) %>% 
  dplyr::select(`Facility Name (Facility Number)`, `Latitude/Longitude`,`Permit Number`,`Permit Type and Description`, `Stream River Mile`) %>% 
  dplyr::arrange(`Facility Name (Facility Number)`) %>% 
...
if(NROW(section.npdes.gen.1)>0){
  ...
  t55_gen1_tbl <- knitr::kable(section.npdes.gen.1[,-3], format = "pandoc", padding = 2,
...
if(NROW(t54_sp)>0){
  ...
  t54_sp_tbl <- knitr::kable(t54_sp, format = "pandoc", padding = 2, row.names = NA,
                             caption = tbls(name = "t54_sp",
                                            caption = paste0("Instantaneous flow measurements by DEQ and others in the ", qapp_project_area_wms_1, ".")))
...
DEQ contracted with Watershed Sciences, Inc. to provide airborne Thermal Infrared Radiometry (TIR) imagery of spatial temperature patterns within the `r qapp_project_area_wms_1` (`r paste0(sort(unique(t52_ref$'Cite_inText')),collapse="; ")`). TIR data is used to characterize the thermal regime of the streams and habitat quality. All streams and the TIR collection dates are summarized in `r tbls(name = "t52", display="cite")`.
...
temp_cal_sites <- model.input %>% 
  ...
  dplyr::filter(Parameter %in% c("Water Temperature","Flow","Effective Shade")) %>% 
...
shade <- t57 %>% 
  dplyr::filter(!`Latitude/Longitude` == "NA/NA") %>% 
  tidyr::separate(`Latitude/Longitude`, into = c("lat","lng"), sep = "/") %>% 
  sf::st_as_sf(coords = c("lng","lat"), crs = sf::st_crs("+init=EPSG:4326"))
