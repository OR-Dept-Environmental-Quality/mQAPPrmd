...
t51.agrimet <- t51 %>% 
  ...
  dplyr::select(-c(`Available Data`,Agency,tbl)) %>% 
  dplyr::add_row(`Station ID` = "CRVO",
                 Station = "CORVALLIS, OREGON AGRIMET WEATHER STATION",
                 `Latitude/Longitude` = "44.6342/-123.19") %>% # add from worksheet
  dplyr::arrange(`Station ID`)
...
t51.sp <- t51 %>% 
  ...
  dplyr::filter(!Source == c("ARAO - AURORA, OREGON AGRIMET WEATHER STATION",
                             "CRVO - CORVALLIS, OREGON AGRIMET WEATHER STATION",
                             "KSLE - MCNARY FIELD AIRPORT, AIRNAV",
                             "KUAO - AURORA STATE AIRPORT, NWS")) # MW
...
# MW flow stations remove from the table (see SharePoint v.55)
keep.flow.stations <- c("Canyon Creek", "Elkhorn Creek", "Sinker Creek", "White Rock Creek", "Roaring River", "Beaver Creek", "Neal Creek at Lulay Bridge near Hannah Covered Bridge", "North Fork Molalla", "SF Crabtree Creek", "Abiqua Creek", "Butte Creek", "Drift Creek", "Little Pudding River")

t54_sp <- t54 %>% 
  dplyr::filter(!`Data Source` %in% c("USGS","OWRD","Hydromet")) %>% 
  dplyr::arrange(`Data Source`,`Station ID`,Station) %>% 
  dplyr::filter(Station %in% keep.flow.stations)
...
This Quality Assurance Project Plan (QAPP) summarizes the modeling approach to be used for the temperature TMDL replacement project applicable within `r knitr::combine_words(unique(sort(huc8.txt$huc8txt)))`, which collectively make up the project area called the Middle Willamette Subbasins. This QAPP project area excludes the Willamette River, the Santiam River, the North Santiam River downstream of Detroit Dam, and the South Santiam River downstream of Foster Dam. These waterbodies are included in the Willamette River and Major Tributaries QAPP. 
...
Stream temperatures for the `r pro.area.tmdls` were simulated using `r if(NROW(tmdl.mod.a)>1){paste0("the computer models")} else {paste0("a computer model")}` (`r knitr::combine_words(tmdl.mod.a$'Model Version')`). `r if(NROW(tmdl.mod.b)>0){paste0("New models are proposed using ", knitr::combine_words(tmdl.mod.b$'Model Version, "."'))}` The model extents include some of the main rivers in the project area that contain or influence primary fish habitat. These models also are tributaries to the series of CE-QUAL-W2 models that are proposed to be used for the Willamette Mainstem and major tributaries temperature TMDL project area. The Heat Source and CE-QUAL-W2 models will not be linked directly since the model years are different, but the temperature outputs of the Heat Source models may be used to develop the boundary conditions temperatures for some of the Wilmette mainstem project area management scenarios. 
