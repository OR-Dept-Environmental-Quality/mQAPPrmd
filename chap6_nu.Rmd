...
# for test: waterbody_name <- "Canton Creek (Summer Period)"
...
`r if(!waterbody_name %in% c("North Umpqua River (Summer period)","North Umpqua River (Spawning period)")){if(waterbody_name %in% waterbody_spawning_nu){paste0("The extent of the model domain is the ", model.domain, " (", figs(name=paste0("tt.fig",waterbody_name),display="cite"),"). The extent differs from the summer model due to data availability.")}else{paste0("The extent of the model domain is the ", model.domain, " The model extent is shown in the HTML interactive map that accompanies this QAPP and is referenced in Appendix D.")}}`
  ...
  if(NROW(t6x6_1)>0){
    ...
    t6x6_1_tbl <-  knitr::kable(t6x6_1, format = "pandoc", padding = 2,
                                caption = tbls(name = paste0("t6x6_1", waterbody_name),
                                               caption = paste0("Boundary condition and tributary inputs to the existing ", waterbody_name_nu, " Heat Source Model.")))
  ...
  if(NROW(t6x7)>0){
    ...
    t6x7_tbl <- knitr::kable(t6x7, format = "pandoc", padding = 2,
                             caption = tbls(name = paste0("t6x7", waterbody_name),
                                            caption = paste0("Calibration sites and parameters used in the existing ", waterbody_name_nu, " Heat Source Model.")))
