...
```{r, label = `sc`, echo=FALSE}

sc <- section.model.info %>% 
  dplyr::select(`Source Characteristics`) %>% 
  dplyr::filter(!is.na(`Source Characteristics`)) %>% 
  pull()

```

`r if(length(sc)>0){sc}else{cat("")}`
...
if(waterbody_name %in% "North Umpqua River"){
  
  cat("The North Umpqua River simulation consists of six separate Heat Source models, separated by each reservoir or diversion dam and Steamboat Creek. Since Heat Source is a one-dimensional model, the reservoirs or backwaters behind the dams were not simulated.")
  cat("\
  
      ")
  cat("\n")
  cat("Model 1: Lemolo Reservoir to Lemolo Powerhouse #1")
  cat("\
  
      ")
  cat("\n")
  cat("Model 2: Lemolo Powerhouse #1 to Toketee Reservoir")
  cat("\
  
      ")
  cat("\n")
  cat("Model 3: Toketee Reservoir to Slide Powerhouse")
  cat("\
  
      ")
  cat("\n")
  cat("Model 4: Slide Powerhouse to Soda Springs Reservoir")
  cat("\
  
      ")
  cat("\n")
  cat("Model 5: Soda Springs Reservoir to Steamboat Creek")
  cat("\
  
      ")
  cat("\n")
  cat("Model 6: Steamboat Creek to Mouth")
  
} else {
  
  cat("The extent of the model domain is the ", model.domain)
...
if(waterbody_name %in% "North Umpqua River"){
  ...
  cat("The model period for North Umpqua Models 1-5 is July 8-11, 2001.")
  cat("\
  
      ")
  cat("\n")
  cat("The model period for North Umpqua Model 6 is July 12-31, 2002.")
  
} else {
  
  cat(paste0("The model period is ",format(as.Date(as.numeric(unique(section.model.info$"Model Period Start")), origin = "1899-12-30"), format="%B %d, %Y"), " to ", format(as.Date(as.numeric(unique(section.model.info$"Model Period End")), origin = "1899-12-30"), format="%B %d, %Y"), "."))
...
**[Update this section if any changes are proposed to the current condition calibration model]**. The effort currently described in the QAPP includes use of existing models with no changes to the models since their previous calibration. Key calibration assumptions made during the model setup and calibration process were documented in the original TMDL `r paste0("(", knitr::combine_words(strip_alpha(unique(sort(model.info[is.na(model.info$'Reference Report'),]$"Abbreviated Reference")))), ")")` and model user guide `r knitr::combine_words(strip_alpha(unique(sort(unique(tmdl.mod.2$mod_ref_intext)))))`.
...
`r if(!model.extent == TRUE){} else {paste0("Model calibration was completed by DEQ and described in the ",paste0(unique(section.model.info[is.na(section.model.info$'Reference Report'),]$'TMDL Document')," (",strip_alpha(unique(section.model.info[is.na(section.model.info$'Reference Report'),]$'Abbreviated Reference')), "). Temperature, flow, and effective shade calibration sites are summarized in "),tbls(name = paste0("t6x5", waterbody_name),display="cite")," to improve documentation of the TMDL approach. The current condition calibration may be updated to reflect spawning period conditions. Potential model calibration adjustments are described in Section 6.2 General model inputs and parameters. ", mod.loc[1])}`
