...
# Include Columbia River in this QAPP
...
subbasin <- unique(lookup.huc[which(lookup.huc$QAPP_Project_Area == qapp_project_area),]$HUC12_Name)
...
station.awqms <- awqms.stations.temp %>% 
  dplyr::filter(HUC12 %in% subbasin_num) %>% 
  dplyr::rename(`Station ID` = MLocID)

station.awqms_usgs_or <- station.awqms %>% 
  ...
  dplyr::mutate_at("StationDes", str_replace_all, "Ordeq", "ORDEQ") %>%
  dplyr::mutate_at("StationDes", str_replace_all, " Rm", " RM") %>%
  dplyr::mutate_at("StationDes", str_replace_all, "Lb", "LB") %>%
  dplyr::mutate_at("StationDes", str_replace_all, "Nf", "NF") %>%
  dplyr::mutate_at("StationDes", str_replace_all, "Sf", "SF")
...
station.owrd <- owrd.stations %>%
  dplyr::filter(HUC8 %in% unique(lookup.huc[which(lookup.huc$HUC12 %in% subbasin_num),]$HUC_8)) %>% 
...
station.usgs.fl <- usgs.fl.stations %>%  # Discharge [ft3/s]
  ...
  dplyr::filter(huc_cd %in% unique(lookup.huc[which(lookup.huc$HUC12 %in% subbasin_num),]$HUC_8)) %>%
...
station.hydromet.flow <- hydromet.stations %>% 
  dplyr::filter(sf::st_contains(pro_area_huc12_union, ., sparse = FALSE)) %>% 
  sf::st_drop_geometry() %>% 
...
flow.stations <- flow.stations.cmb %>% 
...
# _ Water Level data ----
# for Willamette mainstem only
station.usgs.wl <- usgs.wl.stations %>% 
  ...
  dplyr::mutate_at("Station", str_replace_all, " R ", " River ") %>% 
  dplyr::mutate(lat = Lat, lng = Long) %>% 
  sf::st_as_sf(coords = c("lng","lat"), crs = sf::st_crs("+init=EPSG:4269")) %>% 
  dplyr::filter(sf::st_contains(pro_area_huc12_union, ., sparse = FALSE)) %>% 
  sf::st_drop_geometry()

data.usgs.wl <- usgs.wl.data.or %>% 
  dplyr::filter(site_no %in% station.usgs.wl$`Station ID`) %>% 
  dplyr::select(`Data Source` = agency_cd,
                `Station ID` = site_no,
                dateTime,
...
# _ NCDC met data ----
ncei.stations.huc12 <- ncei.stations %>% 
  filter(sf::st_contains(pro_area_huc12_union, ., sparse = FALSE)) %>% 
  sf::st_drop_geometry()

ncei.station.tbl <- ncei.stations.huc12 %>%
...
# _ RAWS met data ----
raws.stations.huc12 <- raws.stations %>% 
  filter(sf::st_contains(pro_area_huc12_union, ., sparse = FALSE)) %>% 
  sf::st_drop_geometry()
...
raws.station.tbl <- raws.stations.huc12 %>% 
  dplyr::left_join(raws.station.data.type, by = "wrccID")

# _ USBR AgriMet ----
agrimet.stations.huc12 <- agrimet.stations.or %>% 
  filter(sf::st_contains(pro_area_huc12_union, ., sparse = FALSE)) %>% 
  sf::st_drop_geometry()
...
agrimet.station.tbl <- agrimet.stations.huc12 %>% 
  dplyr::left_join(agrimet.station.data.type, by = "siteid")

# _ USBR Hydromet ----
hydromet.stations.huc12 <- hydromet.stations %>% 
  filter(sf::st_contains(pro_area_huc12_union, ., sparse = FALSE)) %>% 
  sf::st_drop_geometry()

hydromet.station.tbl <- hydromet.stations.huc12

# _ MesoWest climate data ----
mw.stations.huc12 <- mw.stations %>% 
  filter(sf::st_contains(pro_area_huc12_union, ., sparse = FALSE)) %>% 
  sf::st_drop_geometry()

mw.station.tbl <- mw.stations.huc12 #%>% 
...
load("//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/model_QAPPs/R/data/RData/lookup.RData")

qapp_project_area <- "Willamette River Mainstem and Major Tributaries"
pro_area_huc12 <- sf::read_sf(dsn = "//deqhq1/TMDL/Planning statewide/Temperature_TMDL_Revisions/GIS/willa_snake/TempTMDL_QAPP_Reaches_data_query_HUC12s.shp",
                              layer = "TempTMDL_QAPP_Reaches_data_query_HUC12s") %>% 
  dplyr::filter(Project_Na == qapp_project_area) %>% 
  sf::st_transform(crs = sf::st_crs("+init=EPSG:4326")) #4326 4269
